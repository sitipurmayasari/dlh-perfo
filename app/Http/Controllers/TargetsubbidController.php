...
class TargetsubbidController extends Controller
{
    public function index(Request $request)
    {
        $data = Targetsubbid::orderBy('id','desc')
                        ->SelectRaw('zo_targetsubbid.*')
                        ->leftjoin('zo_subbidang','zo_subbidang.id','zo_targetsubbid.subbidang_id')
                        ->when($request->keyword, function ($query) use ($request) {
                            $query->where('zo_subbidang.name','LIKE','%'.$request->keyword.'%')
                                ->orWhere('zo_targetsubbid.filename', 'LIKE','%'.$request->keyword.'%');
                        })
                        ->paginate('10');
    ...
    {
        Targetsubbid_detail::where('targetsubbid_id',$id)->delete();
